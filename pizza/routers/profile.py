...
from  pizza.models.schemas import ShowProfile,CreateProfile,DisplayProfile
from typing import List
from sqlalchemy.orm import Session
from fastapi_jwt_auth import AuthJWT
from pizza.models.database import get_db,ProfileImage
from pizza.repository.profile import views
import shutil 
...
def create_profile(request:CreateProfile,db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
    try:
    ...
    return views.create_profile(request,db)
...
def get_all_profile(db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Not Authorized, You need to authenticate your access token")
    return views.get_all_profile(db)
...
def show_profile(id,db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Not Authorized, You need to authenticate your access token")
    return views.show_profile(id,db)
...
def update_profile(id,request:CreateProfile,db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Not Authorized, You need to authenticate your access token")
    return views.update_profile(id,request,db,Authorize)
...
def delete_profile(id,db:Session = Depends(get_db),Authorize:AuthJWT = Depends()):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Not Authorized, You need to authenticate your access token")
    return views.delete_profile(id,db,Authorize)
